...
// フォント構造体を初期化
void InitFontx(FontxFile *fx, const char *f0, const char *f1)
{
	AddFontx(&fx[0], f0);
	AddFontx(&fx[1], f1);
}
...
// フォント構造体の表示
void DumpFontx(FontxFile *fx)
{
	for(int i=0;i<2;i++) {
		printf("fx[%d]->path=%s\n",i,fx[i].path);
		printf("fx[%d]->opened=%d\n",i,fx[i].opened);
		printf("fx[%d]->fxname=%s\n",i,fx[i].fxname);
		printf("fx[%d]->valid=%d\n",i,fx[i].valid);
		printf("fx[%d]->is_ank=%d\n",i,fx[i].is_ank);
		printf("fx[%d]->w=%d\n",i,fx[i].w);
		printf("fx[%d]->h=%d\n",i,fx[i].h);
		printf("fx[%d]->fsz=%d\n",i,fx[i].fsz);
		printf("fx[%d]->bc=%d\n",i,fx[i].bc);
...
bool GetFontx(FontxFile *fx, uint8_t ascii , uint8_t *pGlyph, uint8_t *pw, uint8_t *ph)
{
	...
	for(i=0; i<2; i++){
	//for(i=0; i<1; i++){
		if(!OpenFontx(&fx[i])) continue;
		if(FontxDebug)printf("[GetFontx]openFontxFile[%d] ok\n",i);
	
		//if(ascii < 0xFF){
			if(fx[i].is_ank){
				if(FontxDebug)printf("[GetFontx]fx.is_ank fx.fsz=%d\n",fx[i].fsz);
				offset = 17 + ascii * fx[i].fsz;
				if(FontxDebug)printf("[GetFontx]offset=%"PRIu32"\n",offset);
				if(fseek(fx[i].file, offset, SEEK_SET)) {
					printf("Fontx:seek(%"PRIu32") failed.\n",offset);
					return false;
				}
				if(fread(pGlyph, 1, fx[i].fsz, fx[i].file) != fx[i].fsz) {
